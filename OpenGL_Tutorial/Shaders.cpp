...
int main()
{
	...
	// code from the website binds the VAO again. That seems unecessary to me
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		// update shader uniform with new shade of green
		float timeValue = (float) glfwGetTime();
		float greenValue = (sin(timeValue) / 2.0f) + 0.5f;
		// connect the global uniform with the linked shader program
		int vertexColorLocation = glGetUniformLocation(shaderProgram, "ourColor");
		// C doesn't have overloaded methods so 4f is the type of glUniform with four floats
